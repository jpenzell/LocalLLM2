...
# Global variable for the Gradio interface
iface = None
server = None  # Add global server variable
...
def cleanup():
    ...
    try:
        if iface:
            logging.info("Closing Gradio interface...")
            iface.close()
            iface = None
        if server:
        ...
        # Kill any other instances of our app
        import psutil
        current_pid = os.getpid()
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                # Kill any other Python process running app.py (but not ourselves)
                if (proc.info['cmdline'] and 
                    any('app.py' in cmd for cmd in proc.info['cmdline']) and 
                    proc.info['pid'] != current_pid):
                    logging.info(f"Killing other process {proc.info['pid']}")
...
def update_model_selector():
    """Update the model selector dropdown with all models"""
    installed_models = get_installed_models()
    available_models = fetch_available_models()
...
def generate_response(prompt: str, history: List[List[str]], system_prompt: str, temperature: float, model: str) -> str:
...
def create_gradio_interface():
    with gr.Blocks() as iface:
        gr.Markdown("# LocalGPT Interface")
        ...
        def chat_interface(message, history, system_prompt, temperature, model):
...
if __name__ == "__main__":
    try:
        ...
        logging.info("Enabling queue...")
        iface.queue()
        logging.info("Starting server on http://127.0.0.1:7860")
        ...
        import uvicorn
        config = uvicorn.Config(app=iface.app, host="127.0.0.1", port=7860)
